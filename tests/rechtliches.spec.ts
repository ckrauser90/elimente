...
test.describe('Rechtliche Seiten', () => {
  ...
  test('13.5 Impressum enthält Pflichtangaben', async ({ page }) => {
    ...
    // Name, Adresse, E-Mail und Telefon prüfen
...
test.describe('Barrierefreiheit & Performance', () => {
  ...
  test('15.3 Seite lädt ohne JS-Konsolenfehler', async ({ page }) => {
    ...
    // Bekannte unkritische Fehler ignorieren (kein Backend/Config in Tests)
    const relevanteErrors = errors.filter(e =>
      !e.includes('supabase') &&
      !e.includes('SUPABASE') &&
      !e.includes('fetch') &&
      !e.includes("Unexpected token '<'") // config.local.js nicht vorhanden – erwartet
    );
